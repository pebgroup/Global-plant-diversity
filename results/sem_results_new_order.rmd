...
ggplot(dat_no.na, aes(x=tra_m, y=sr_trans, col=mat_breaks))+
  ...
  ylab("species richness")
,
ggplot(dat_no.na, aes(x=sea_m, y=sr_trans, col=mat_breaks))+
  geom_point()+
   scale_color_viridis_c(option = "plasma", 'MAT intervals')+
  geom_smooth(method = "lm", se=F, col="grey")+
  geom_smooth(method = "lm", aes(group=mat_breaks), se=F)+
...
plot(p)
#ggsave("simpsons_paradox_SR.png", p, width=8, height=5, units = "in", dpi = 600)
```
...
plot(p)
#ggsave("simpsons_paradox_MRD.png", p, width=8, height=5, units = "in", dpi = 600)
...
png("slope_mat.png",width=3, height=3,units="in", res=200)
plot(res[,1]~mat_mean, col=c("red", "black")[as.numeric(res[,2]<0.05)+1], 
     xlab="mat_m", ylab="bin regression slope", pch=20)
abline(h=0)
dev.off()
...
res <- cbind(res, c(1:10))
res <- as.data.frame(res)
dat_no.na <- merge(dat_no.na, res, by.x="mat_breaks", by.y="V3", all.x=TRUE)
shp <- merge(shp, dat_no.na[,c("level3","Estimate", "Pr(>|t|)")], by.x="LEVEL_3_CO", by.y="level3", all.x=TRUE)
names(shp)[grepl("Pr", names(shp))] <- "pvalue"
...
shp <- merge(shp, dat_no.na[,c("level3","mat_m")], by.x="LEVEL_3_CO", by.y="level3", all.x=TRUE)
shp$Estimate2 <- shp$Estimate
shp$Estimate2[shp$pvalue>0.05] <- NA
(my_plot3 <- ggplot(shp) + 
  geom_sf(aes(fill = Estimate2), lwd=0.1) + 
  scale_fill_viridis_c(option = "plasma", "Regression slope")+ 
...
# SA in residuals
On https://github.com/jebyrnes/spatial_correction_lavaan there is code to 

* fix the no residuals problem (no latent varaible gives all residuals being zero)
* is a lacSpatialCorrect.R function
* correct residual estimation: residuals_lavaan()
*predict_lavaan()

## visualize residuals and fitted SEM values
```{r}
...
#as.data.frame(residuals(c2.fit.p, "casewise")) # does not work cause not latent variable
...
#cor.test(shp$sr_trans_fitted,shp$sr_trans)
,
...
ggplot(shp, aes(x=abs(lat), y=sr_trans_residuals, col=sr_trans))+
  geom_point()+
  geom_smooth(method="lm")
# ggplot(shp, aes(x=lat_bin, y=sr_trans))+
#   geom_boxplot()
# ggplot(shp, aes(x=lat_bin, y=sr_trans_residuals))+
#   geom_boxplot()
)
...
# Correct SA using Byrrnes function 
Code by Jarrett Byrnes https://github.com/jebyrnes/spatial_correction_lavaan/tree/bc60fd4e44d97c56994c32719d6d1a157ff22b5d
lavSpatialCorrect() uses MoransI function from ape: This function computes Moran's I autocorrelation coefficient of x giving a matrix of weights using the method described by Gittleman and Kot (1990).

The correction he suggests is based on the global distance matrix, think thats not appropriate because our data has most autocorrelation in neighboring countries. Wrote a second function to correct for custom distance bands.
...
# try another distance band. 1600 is ideal for raw SR, 1000 is ideal for SR residuals
moran$dist.class[which.max(moran$moransI_sem)]
c2.fit.SA.1600 <- my.lavSpatialCorrect(c2.fit.p, dat_no.na$x, dat_no.na$y, distance = 1000)
c2.fit.SA.1600 # all p values are the same, but thats a genuine signal (look distance band plot)
round(c2.fit.SA.1600$parameters$sr_trans[,c(2:6)],3)
round(c2.fit.SA.1600$parameters$mrd[,c(2:6)],3)
round(c2.fit.SA.1600$parameters$soil[,c(2:6)],3)
round(c2.fit.SA.1600$parameters$sub_trop_mbf[,c(2:6)],3)

```

<!--
# Correct SA, adjusted function
```{r}
# CORRECTED FUNCTION
c2.fit.p.cor.my <- my.lavSpatialCorrect(c2.fit.p, dat_no.na$x, dat_no.na$y, distance = 1600)
c2.fit.p.cor
# that seems all mixed up, the parameters dont fit at all...???

# test correction
resids.c <- as.data.frame(residuals_lavaan(c2.fit.p.cor))
# cannot get residuals for that??
S.dist  <-  dnearneigh(coo, 0, 1600, longlat = TRUE) # define search radius. we got decimal degrees, so the number refers to kilometers
# nb <- poly2nb(s1, queen=TRUE) # only neighbors 
s1$LEVEL_3_CO[S.dist[[1]]] # neighboring counties
lw <- nb2listw(S.dist, style="W",zero.policy=T)
MI  <-  moran.mc(s1$sr_trans, lw, nsim=599,zero.policy=T)
...
```
No spatial autocorrelation in residuals, some spatial autocorrelation in RAW SR. Values are MUCH smaller than when using spdep
-->
...
resids <- as.data.frame(residuals_lavaan(c2.fit.p)) # MAKE SURE THIS IS THE RIGHT ORDER
cbind(rawdata$mrd, dat_no.na$mrd) # this is the right order
names(resids) <- paste0(names(resids),"_residuals")
resids$level3 <- dat_no.na$level3 
s1$mrd_residuals <- resids$mrd_residuals

coo <- coordinates(s1) # get center of each polygon
moran_mrd <- data.frame(dist.class = seq(100, 10000, 100),
                    moransI = NA,
                    moransp = NA,
                    moransI_sem = NA,
                    moransp_sem = NA)
for(i in 1:length(moran$dist.class)){
  ...
  moran_mrd$moransI[i] <- MI$statistic
  moran_mrd$moransp[i] <- MI$p.value
  MI2 <- moran.mc(s1$mrd_residuals, lw, nsim=599,zero.policy=T) 
  moran_mrd$moransI_sem[i] <- MI2$statistic
  moran_mrd$moransp_sem[i] <- MI2$p.value
  if(!i%%1)cat(i,"\r")
}

plot(moran_mrd$moransI~moran_mrd$dist.class, 
     ylim=c(-0.05,0.8),
     type="p", pch=20,
     col=c("red", "black")[as.numeric(moran$moransp<0.05)+1],
     xlab="Distance class (km)",
     ylab="Moran's I")
points(moran_mrd$moransI_sem~moran_mrd$dist.class,
      col= c("red", "black")[as.numeric(moran_mrd$moransp<0.05)+1],
      pch=17)
abline(h=0, col="grey30", lty=2)

#raw visualization of MRD residuals
shp <- merge(shp, resids, by.x="LEVEL_3_CO", by.y="level3", all.x=TRUE)
#shp <- merge(shp, dat_no.na[,c("mrd", "level3")], by.x="LEVEL_3_CO", by.y="level3", all.x=TRUE)
plot(shp$mrd_residuals~shp$mrd.x) 
cor.test(shp$mrd_residuals,shp$mrd.x) 
abline(h=0, col="red")
```
