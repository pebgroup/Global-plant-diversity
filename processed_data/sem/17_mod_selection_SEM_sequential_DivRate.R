...
fixed_sr <- c("sr_trans ~ soil + sub_trop_mbf + area + mdr +")
fixed_mdr <- c("mdr ~ mat_m + mio_mat_ano_m + boreal_f_taiga +")
...
names(selected_variables[[3]])
mdr_variables <- c( "prs_m", "tra_m", "mat_lgm_ano_m", "temp_bmf", "soil",
                    "prs_sd", "temp_cf", "area", "medit_fws", "mio_pre_ano_m")
...
print("Building mdr equations...")
mdr_var_list <- do.call("c", lapply(seq_along(mdr_variables[1:var.max]),
                                    function(i) combn(mdr_variables[1:var.max], i, FUN = list)))
mdr_combs <- c()
for(i in 1:length(mdr_var_list)){ #
  temp <- unlist(mdr_var_list[i]) 
  mdr_combs <- c(mdr_combs, str_c(temp, collapse = "+"))
}


# build all options combining the two regression forumulas
temp <- expand.grid(sr_combs, mdr_combs)
main_reg = paste(paste(fixed_sr,temp$Var1), paste(fixed_mdr, temp$Var2), sep="\n")
...
print(paste0("Saving files as SEM_results_", nstart, "_", nmods, ".RData"))
save(temp, file=paste0("output/SEM_results_DivRate_", nstart, "_", nmods, ".RData"))
