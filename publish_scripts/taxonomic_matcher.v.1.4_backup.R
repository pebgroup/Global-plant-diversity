  ...
  if(DB.name=="GBIF"){
    ...
    sb_input <- readRDS("data/input_tip_labels_apg.rds")
    ...
    # rename
    sb_input$taxon_rank <- gsub("variety", "var", sb_input$taxon_rank, fixed=TRUE)
    sb_input$taxon_rank[which(is.na(sb_input$taxon_rank))] <- "undefined"
    
    classes <- as.vector(unlist(lapply(sb_input, class)))
    sb_input[,which(classes=="factor")] <- lapply(sb_input[,which(classes=="factor")], as.character)
    
    # exclude Bryophyta
    bry <- read.csv("data/bryophyta.csv")
    bryos <- as.character(bry[,1])
    bryos <- gsub(" ", "", bryos)
    sb_input <- sb_input[!sb_input$family.apg %in% bryos,]
    
    # exclude taxa without extracted genus
    if(any(is.na(sb_input$genus))){
      sb_input <- sb_input[-which(is.na(sb_input$genus)),]      
    }
  ...
  ## subset wcsp to species occurring in dataset
