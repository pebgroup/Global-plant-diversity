...
# next script: sem_results_new_order.rmd
rm(list=ls())
library(lavaan)
library(stringr)
library(ggplot2)
...
# sr_variables <- c("sub_trop_dbf",  "pre_sd", "mont_gs", "mat_sd", "tri", "sea_sd", "tra_m", "tra_sd", "pre_m", "pet_sd", 
#                   "sea_m", "mat_m") 
sr_variables <- c("sub_trop_dbf",  "pre_sd", "mont_gs", "mat_sd", "sea_sd", "tra_m", "pre_m", "tri", "pet_sd", "tra_sd",
                  "mat_m", "pet_m", "sea_m")
# tri + pet_sd are new, sea_m + mat_m missing. pet_m is naturally excluded
# mrd_variables <- c("tri", "mat_m", "soil", "pet_m", "sea_sd", "pre_sd", "area", "temp_bmf", "tra_sd", "pet_sd", 
#                    "mat_sd", "sub_trop_mbf") 
mrd_variables <- c("tri", "soil", "tra_sd", "pre_sd", "temp_bmf", "pet_m", "mat_m", "sea_sd", "pet_sd", "area",  
                   "mat_sd", "sub_trop_mbf")
# pet_sd and temp_bmf are new, subtropdbf is missing
fixed_sr <- c("sr_trans ~ soil + sub_trop_mbf + area + mrd +")
fixed_mrd <- c("mrd ~ pre_m + sea_m + tra_m +")
...
# EXTRACT STATS ####
load("data/brute_force_mod_selection_results_parallel_mat_new_order.RData")
fit <- sapply(temp, "[", 1)
...
# model fit
length(which(fit2$cfi.robust>0.9)) #202 models # 300 more models than using pet_m
length(which(fit2$rmsea.robust<0.12)) # 15  # 14 with 10 variables
...
cor(fit2$cfi.robust[fit2$aic<2800], fit2$aic[fit2$aic<2800]) # thats also more than with using pet_m
...
# last one makes AIC go up again

cat(best_cfi_mod5)
#summary(best_cfi.fit.mod5, standardized = TRUE, fit.measures=TRUE, rsq=TRUE)

png("best_cfi_mod_MAT_new_order.png", width=900, height=500, units = "px")
par(mfrow=c(1,2))
...
semPaths(object = best_cfi.fit.mod5, layout = "circle2", rotation = 1,
         whatLabels="std", label.cex = 1.5, edge.label.cex = 1, what = "std", 
         exoCov = FALSE, exoVar = FALSE,
         nCharNodes = 0, fade=FALSE,
         edgeLabels = sem_sig_labels(best_cfi.fit.mod5))
title(round(fitmeasures(best_cfi.fit.mod5, c("cfi.robust", "rmsea.robust", "aic")),3), cex.main=1)
dev.off()
...
png("best_rmsea_mod_MAT_new_order.png", width=900, height=500, units = "px")
par(mfrow=c(1,2))
...
# like best CFI, but without tra_m --> MRD path
...
best_global_aic.mod8 <- paste0(best_global_aic.mod7, "\nsoil~tri")
best_global_aic.mod.fit8 <- sem(best_global_aic.mod8, data = dat_no.na, estimator="MLM")
fitmeasures(best_global_aic.mod.fit8, c("chisq", "df", "pvalue", "cfi.robust", "rmsea.robust", "aic"))
modificationindices(best_global_aic.mod.fit8, sort. = TRUE)[1:25,]
best_global_aic.mod9 <- paste0(best_global_aic.mod8, "\nsr_trans~pre_m")
best_global_aic.mod.fit9 <- sem(best_global_aic.mod9, data = dat_no.na, estimator="MLM")
fitmeasures(best_global_aic.mod.fit9, c("chisq", "df", "pvalue", "cfi.robust", "rmsea.robust", "aic"))
# no more changes in AIC while RMSEA is way too high already

png("best_global_aic_mod_MAT_new_order.png", width=900, height=500, units = "px")
par(mfrow=c(1,2))
...
semPaths(object = best_global_aic.mod.fit8, layout = "circle2", rotation = 1,
         whatLabels="std", label.cex = 1.5, edge.label.cex = 1, what = "std", 
         exoCov = FALSE, exoVar = FALSE,
         nCharNodes = 0, fade=FALSE,
         edgeLabels = sem_sig_labels(best_global_aic.mod.fit8))
title(round(fitmeasures(best_global_aic.mod.fit8, c("cfi.robust", "rmsea.robust", "aic")),3), cex.main=1)
dev.off()
...
nrow(fin_good)
# 133 # 609 for 0.13 and pet_m, 722 for mat_m
...
fin_good_aic_min <- fin_good_aic[which(fin_good_aic$var.n==m.aic),] # 6 good models
...
modificationindices(best_aic.fit.mod3, sort. = TRUE)[1:25,]
...
png("best_aic_mod_MAT_new_order.png", width=900, height=500, units = "px")
par(mfrow=c(1,2))
...
cat(best_abs_aic_mod4)

png("best_absolute_aic_mod_MAT_new_order.png", width=900, height=500, units = "px")
par(mfrow=c(1,2))
...
semPaths(object = best_abs_aic.fit.mod4, layout = "circle2", rotation = 1,
         whatLabels="std", label.cex = 1.5, edge.label.cex = 1, what = "std", 
         exoCov = FALSE, exoVar = FALSE,
         nCharNodes = 0, fade=FALSE,
         edgeLabels = sem_sig_labels(best_abs_aic.fit.mod4))
title(round(fitmeasures(best_abs_aic.fit.mod4, c("cfi.robust", "rmsea.robust", "aic")),3), cex.main=1)
dev.off()
...
nrow(fin_good_max.var) # 5 good models with max var

cat(fin_good_max.var$model.name)
# any moulticolineariy? no. take the one with best fit
fin_good_max.var[,-7]
...
modificationindices(max.var.mod.fit.mod3, sort. = TRUE)[1:20,]
...
modificationindices(max.var.mod.fit.mod4, sort. = TRUE)[1:20,]
...
modificationindices(max.var.mod.fit.mod5, sort. = TRUE)[1:20,]
...
modificationindices(max.var.mod.fit.mod6, sort. = TRUE)[1:20,]
...
modificationindices(max.var.mod.fit.mod7, sort. = TRUE)[1:20,]
...
png("max_var_MAT_new_order.png", width=900, height=500, units = "px")
par(mfrow=c(1,2))
...
# one more variable (tri) and despite that still a lower AIC
...
save.image("brute_force_models_new_order.RData")
