...
# SR MAP ##########################################################################
library(scico) # try lapaz + batlow
# ggplot(shp) + 
#   geom_sf(aes(fill = sr), lwd=0.1) + 
#   scale_fill_scico(palette = "lapaz", trans="sqrt")+
...
#   geom_smooth(data=shp[shp$lat<0,], method="lm", col="#CC0C00FF")+
#   geom_smooth(data=shp[shp$lat>0,], method="lm", col="#5C88DAFF")
# )
...
#           panel.grid.minor = element_blank(),
#           panel.border = element_blank())+
#     theme(axis.line.x = element_line(size = 0.1, colour = "black"))
# )
