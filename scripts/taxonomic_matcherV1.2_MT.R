  ...
  if(DB.name=="BIEN"){
    ...
    input <- readRDS("data/input_vectorized3.rds")
    
    # RENAMING BIEN
    
    # straighten formats
    input$taxon_rank[which(input$taxon_rank=="[unranked]")] <- NA
    input$taxon_rank <- gsub(".", "", input$taxon_rank, fixed=TRUE)
    
    # rename
    input$taxon_rank <- gsub("cv", "convar", input$taxon_rank, fixed=TRUE)
    input$taxon_rank <- gsub("fo", "f", input$taxon_rank, fixed=TRUE)
    input$taxon_rank <- gsub("ssp", "subsp", input$taxon_rank, fixed=TRUE)
    input$taxon_rank <- gsub("unranked", NA, input$taxon_rank, fixed=TRUE)
    input$taxon_rank[which(is.na(input$taxon_rank))] <- "undefined"
  
    input[,1:3] <- lapply(input[,1:3], as.character)
    
    # exclude Bryophyta
    bry <- read.csv("data/bryophyta.csv")
    bryos <- as.character(bry[,1])
    bryos <- gsub(" ", "", bryos)
    input <- input[!input$family %in% bryos,]
  ...
  if(DB.name=="GBIF"){
    ...
    input <- readRDS("data/input_tip_labels.rds")
    ...
    input[,c(1:4,6:10)] <- lapply(input[,c(1:4,6:10)], as.character)
  ...
  # saveRDS(unsolved, file=paste0("data/unsolved_", DB.name , ".rds"))
  # 
  # # assign multimatch 
  ...
  # save output
  saveRDS(fin, file=paste0("data/fin_", DB.name , ".rds"))
  ...
  # save output
  saveRDS(fin_species_match, file=paste0("data/fin_species_match_", DB.name, ".rds"))
